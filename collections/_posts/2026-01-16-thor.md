...
<div style="text-align:center">
<img src="/collections/images/thor/VAE-procedure.jpg" width=300></div>

### Diffusion process

* A standard diffusion process is used to learn the reconstruction of images from healthy subjects
...
$$\hat{x}^t_0 = V_{D,\phi}(\hat{z}^t_0) = V_{D,\phi}(\frac{1}{\sqrt{\bar{\alpha}_t}} \left(z_t - \sqrt{1-\bar{\alpha}_t}\, \epsilon_\theta(z_t,t) \right))$$

  where $$V_{D,\phi}$$ is the pre-trained VAE decoder

* These maps critically compare the predictive reconstructions $$\hat{x}^t_0$$ with the actual input image $$x^{\text{input}}_0$$, highlighting discrepancies that indicate anomalies and distinguishing regions that are likely healthy
...
* These anomaly maps are then used in the "harmonization" process to produce reconstructions that not only closely
resemble the original images but also conform to the healthy tissue profile

$$x_t = cd\left( m(\hat{x}^t_0,x^{\text{input}}_0) \right) \cdot \hat{x}^t_0 + \left( 1 - cd\left( m(\hat{x}^t_0,x^{\text{input}}_0) \right) \right) \cdot x^{\text{input}}_0$$

* $$x_t$$ is then used to [...] <font color="red">voir avec Celia</font>
