...
As CLIP[^1] is a foundation model in natural langage processing that can generalize to tasks and data distributions beyond those seen during training, Segment Anything Model (SAM) tend to be a foundation model in segmentation task as there are only fine-tuned model in this domain. This model is a promptable model pretrained on a large dataset that enables powerful generalization without transfer learning. To do so a data engine is used to train and create a new large dataset.

![](/collections/images/SAM/graphical_abstract.png)
...
![](/collections/images/SAM/input_ambiguity.png)

Usually promptable and interactive segmentation uses a sequence of inputs to eventually predict a valid mask, the goal here is to have a valid mask for any prompt even when it is ambiguous. The pre-trained model must be effective in those cases which is challenging and requires specialized modeling and training loss choices. The good prompt engineering must be found to success a zero-shot transfer to any task linked to segmentation that are interactive segmentation, edge detection, super pixelization, object proposal generation, foreground segmentation, semantic segmentation, instance segmentation, panoptic segmentation, etc. It differs from a multiple-task segmentation model as it does not have a fixed set of tasks but can be applied to a new task it was not trained for thanks to prompting and composition.

# Segment Anything Model

![](/collections/images/SAM/SA_model.png)

The image encoder is a Masked AutoEncoder (https://creatis-myriad.github.io/2022/08/31/MAE.html) pre-trained Vision Transformer (ViT) adaptated to have high resolution input.

The prompt encoder is separated between sparse (points, boxes, text): where positional encoding, or text encoding with CLIP, is summed with embeddings learned for each prompt type and dense (mask): where imputs are embedded using convolutions and summed element-wise with the image embedding.
...
SAM is first trained with a public annotated dataset and professional annotaters use it with foreground/background clicks and reajust the masks with brush and earaser tools. They annotate as many objects as possible in the images in order or prominence. The model is retrained 6 times while the quantity of data augment without the public dataset as soon as there are enough new annotated images.

## Semi automatic annotation

This second step is to increase the data diversity. This time SAM segments first the images trained with boxes and with the category "object" and then the annotators add new masks to the images. It is retrained 5 times, the number of masks increase from 44 to 72 per image.
...
Mean resolution of images is 3300x4950 pixels and even with downsampling the images, it is better quanlity than others vision datasets. 99.1% of masks are generated automatically but the final SA-1B only contains automatically generated masks.

![](/collections/images/SAM/dataset_mask_properties.png)

To assess the quality of the masks, 500 images are sampled (~50,000 masks) and annotaters correct the masks with brush and earaser. Comparing raw masks and corrected ones, 94% of the automatically generated masks have an IoU >90% and 97% have an IoU >95%.

# Responsible AI analysis

![](/collections/images/SAM/dataset_geographical_properties.png)

![](/collections/images/SAM/dataset_income_repartition.png)

Top 3 countries are from different regions in the world and even if Africa an low income countries are under represented, there are 28M masks from Africa which is 10 times more than all other datasets together. It would have been interesting to see those numbers in relation to the number of inhabitants and compare it to other dataset regarding the number of masks in them.

![](/collections/images/SAM/SAM_performance_biais.png)
...
![](/collections/images/SAM/test_datasets.png)
...
![](/collections/images/SAM/test_results.png)

Both mIoU and human rating of the masks between 1 to 10 are compared. On 16 out of 23 datasets SAM performs better than RITM but when an oracle selects the best mask according to the ground truth (and not selection of the most confident mask) SAM is better on all dataset. Looking at the datasets on which this change allows a great improvement could be interesting. When human rating is considered, SAM masks are better than RITM's ones even when the IoU is worse. The more there are prompted points, the less SAM is better than the other except when the points are not centered.

## Edge detection

A 16x16 grid of points is applied to all images which leads to 768 masks per image (3 per point). The redundancy is removed with Non Maximal Suppresion (NMS) and a Sobel filter is applied on unthresholded masks probability masks to have edges. 

![](/collections/images/SAM/edge_detection.png)

![](/collections/images/SAM/edge_detection_table.png)
...
![](/collections/images/SAM/object_proposal.png)
...
![](/collections/images/SAM/instance_segmentation.png)

![](/collections/images/SAM/mask_quality_rating_COCO_LVIS.png)
...
![](/collections/images/SAM/text_to_mask.png)

## Ablations

The ablations concern the SA-1B dataset and the architecture of SAM.

![](/collections/images/SAM/ablation_study.png)
...
Models are almost always dedicated to a particular task, but here the foundation model trained on a very large dataset is then adapted to many tasks. However, generally foundation models are auto-supervised wherease here it is initialised with MAE that is based on a supervised work.

The goal of SAM is to be used in composition system (like CLIP in DALL-E) in order to get new capabilities that were not even thought about at its creation. To do so, SAM needs to predict a valid mask for a wide range of segmentation
prompts in order to create a good interface between SAM and other components.

It still has some limitations, there are some errors missing small structures or creating small disconnected components, and does not produce boundaries as crisply as more computationally intensive methods. Given the fact that it is a very generic segmentation model which goal is to perform with zero transfer learning, it is expected that all dedicated interactive segmentation methods to outperform SAM. Another limitation is the computational time: it is said to be real-time but it is not really the case when using a heavy image encoder. Finally even if SAM performs many tasks, it is not easy to design simple prompts that implements semantic and panoptic segmentation.
