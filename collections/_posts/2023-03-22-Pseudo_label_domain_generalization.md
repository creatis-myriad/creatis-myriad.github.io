...
Two images X and X' are randomly drawn from the source domain $$\{B,C,D\}$$. The amplitudes and phase of those images are computed with teh Fourier Transform: $$\mathcal{A}, \mathcal{P}, \mathcal{A}'$$
...
The authors use a **Cross Pseudo Supervision** (CPS) scheme, developped in another paper[^1]: the prediction from one network is considered as a pseudo label and used to supervise the other network, and vice-versa.
...
They also introduce a confidence aware loss function: $$L_{cacps}=L_a+L_b$$ using cross-supervision signals $$L_a = E[e^{-V_1}L_{ce}(P_E^2,Y_1)+V_1]$$. $$L_{ce}$$ is the cross-entropy loss and Y1 is one-hot vector generated by the probability map $$P_E^1$$.
