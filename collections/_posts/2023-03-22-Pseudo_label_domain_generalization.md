...
# Highlights
* The goal of the paper is to obtain better quality pseudo labels for unlabelled images from unknown image domain
...
Domain shifts are very common in the medical field, with for instance datasets that can be generated with scanners produced by different vendors or with images that come from different types of patients. Therefore, when working with an already annotated dataset, acquired in a specific domain, if one wants to apply the developped networks/methods to another dataset, in another domain, it might not be trivial. 
...
The authors of this paper try to do this in an **semi-supervised setting**, where part of their trainig data does not have ground truth labels.
...
Two images X and X' are randomly drawn from the source domain. The amplitudes and phase of those images are computed with teh Fourier Transform: $$\mathcal{A}, \mathcal{P}, \mathcal{A}'$$

**Note:** The original domain of the drawn image are unknown
...
The authors use a **Cross Pseudo Supervision** (CPS) scheme, developped in another paper[^1]: the prediction from one network is considered as a pseudo label and used to supervise the other network.

In the setting introduced by the authors, with an unknown original domainfor the input image (it could be B,C or D), the psuedo labels generated could have a low quality, due to the potentially high variance between samples from the different domains. Therefore, the low quality labels should have less influence: that's their scheme **Confident-Aware Cross Pseudo Supervision** (CACPS).
...
They also introduce a confidence aware loss function: $$L_{cacps}=L_a+L_b$$ using cross-supervision signals $$L_a = E[e^{-V_1}L_{ce}(P_E^2,Y_1)+V_1]$$. Here Y1 is one-hot vector generated by the probability map $$P_E^1$$.
...
During infrerence, both predictions 1 and 2 are used for the final results.
...
- Optimizer : Adamw 
- other data augmentation schemes are used with random rotation, scaling, crop, flip
...
The methods brings better results visually and quantitatively than the baselines, even compared with Meta[^2], which is SOTA for semi-supervised learning and domain adaptation. 
...
- The two schemes introduced by this paper help getting better qualty labels which means better generalization to unknown domains, without the need to have .
