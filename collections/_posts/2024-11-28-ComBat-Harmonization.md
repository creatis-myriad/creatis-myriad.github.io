...
* The authors show that the DTI measurements are highly site-specifics highlighting the need of correcting for site effects before performing downstream statistical analyses. The process of correcting these site-effects is usually call **harmonization**.
...
The study:
* Condiders **two** DTI studies from different scanners.
* Considers participants that are **matched** for age, gender, ethnicity and handedness, resulting in 105 participants retained in each study for both dataset.
* We will mainly **focus on the FA diffusion metric** here, but the authors presented some results for MD, RD and AD in their supplementary materials.
...
* Uses image processing: 
  * Quality control was done manually on each image of each volume, removing signal dropouts likely caused by subject motion and pulsating flow, ghosting artefacts or image stripping.
  * **Registration** on the Eve template.
...
The authors selected a subset of the Philadelphia Neurodevelopmental Cohort (PNC) [Satterthwaite et al., 2014], and included 105 healthy participants from 8 to 19 years old. 83 of the participants were males (22 females), and 75 participants were white (30 non-white). The DTI data were acquired on a 3T Siemens TIM Trio whole-body scanner, using a 32 channel head coil and a twice-refocused spin-echo (TRSE) single-shot EPI sequence with the following parameters: TR = 8100 ms and TE = 82 ms, b-value of 1000 s/mm2, 7 b= 0 images and 64 gradient directions. The images were acquired at 1.875 × 1.875 × 2 mm resolution. During the same session, structural T1weighted (T1-w) MP-RAGE images were also acquired with parameters TR = 1810 ms, TE = 3.5 ms, TI = 1100 ms and FA = 9°, at 0.9375 × 0.9375 × 1 mm resolution.
...
<!-- Show how the compute the estimates with the bayesian framework -->
...
For each silvert-standard two different sets are created since previous studies have shown that some brain regions are more specific in changes for FA and others for MD.
...
2) For each site, they define the site-specific signal silver-standard to be the k=5000 voxels with the highest t-sttistics in magnitude.
...
## DTI scalar maps are higly affected by site
See figure 2
...
All harmonization seem preserving within-site biological variability  except SVA which has a substantially lower correlation than the other methods.
...
* Howether doing nothing will let an undesirable situation happened where site will be a confounder for the relationship between the DTI values and the phenotypes of interest.
  
* Confounding between age and site presents an additional challenge for harmonization, since removing variation associated site can lead to removing variation associated with age if not done carefully.
...
This evaluates the performance of the different harmonizatuion methods at replicating the voxels associated with age across independent datasets.
...
* Negative and qualitative confounding: combining the data without a proper harmonization technique lead to more severe problems. The raw data curve is far below the diagonal line indicating a negative correlation. The authors explain that due to these confounding, the t-statistics for each voxel that are truly not associated with age, normally centered around 0, became higly negative because of the site effect. On the other hand, t-statistics for the voxels associated with age are shifted towards 0. These confounding render the null voxels significant and create a reversed ranking.
...
<div style="text-align:center">
Figure 7 presents the distribution of the estimated effect sizes on the signal silver-standard for all methods and for all confounding scenarios. Th dashed line represents the median of the true effect sizes, and the solid line represents an effect size of 0.
</div>

Results for raw data are consitent: positive (resp. negative) confoundind shifts the effect size positively (resp negatively).
...
* Same observations with the silvert-standards of site 1 and 2 (Figure 8.c and 8.d).
...
Because subsets are created within site, they are not affected by site effects and results obtained from those subsets = be superior or as good as any of the results obtained from the harmonized subsets.
...
* The authors shows that combining the two studies without proper harmonization led to a decrease in power od detecting voxels associated with age.
* This confirmed that DTI measurements are higly affected by small changes in the scanner parameters, as those affect the underlying water diffusitivity.
