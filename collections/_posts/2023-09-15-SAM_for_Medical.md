...
This is a combined review of the four following papers. The aim is to give an idea about how Segment Anything (SAM)[^1] has been used and adapted to medical images since its released in April 2023.
...
* The encoder is a a masked auto-encoder (MAE) pre-trained ViT-H/16 
...
* The decoder is composed of only 2 Transformer's decoder blocks with prompt tokens to image embeddinds and image embeddings to prompt tokens cross-attention. The resulting embeddings are upscaled with transpose convolution to the input image size and an additional MLP predicts the IoU score with the target object.
...
LoRA is an effficient finetuning strategy for large-scale models that is aimed to condense the transformer features to a low rank space and then reproject them to the dimensions of the output features in the freezed transformer blocks. The feature size in the projected LoRA space is divided by 4.
* The updated model only increases the original model size by 5.25% (+18.8M parameters)
* All the parameters in the prompt encoders and the mask decoder are finetuned (LoRA in the mask decoder's transformer blocks was tested but yield lower results).
* 30 abdominal CT scans from the MICCAI 2015 Multi-Atlas Labeling Challenge is used for training/validation and the model is then evaluated on the Synapse multi-organ segmentation dataset.
...
* To better handle 3D images, the authors uses a strategy inspired from image-to-video adaptation:

For a sample DxNxL (with D the depth, N the number of embeddings and L their size), a first multi-head self-attention acts over NxL to learn spatial correlation. Additionaly, the input tensor is transposed to the shape NxDxL and a second multi-head attention learns depth correlation over DxL. 

* MSA also use the pre-trained SAM decoder and adds three Adapters. The first one also integrate the prompt information in the low-dimension space.
...
* The  prompt encoder and the mask decoder are kept as it without any adjustment and their parameters frozen.
* The input spatial size is reduced from 1024x1024 to 256x256 to reduce GPU memory cost.
* The patch embedding has th same parameters as in SAM but *half its stride* to better process local information from patch boundaries. 
...
* Model is trained/validated/tested on US30K, a large ultrasonic dataset containing data from seven publicy-available datasets.
...
Finetuning SAM on medical imaging dataset is essential to deal with the domain shift with natural images. However, more advanced stratagies, such as using low-rank adapter, seem to work better than finetuning all the parameters of the model. Adapted SAM reaches performances that are in-par or superior fully state-of-the-art supervised models on various tasks.

Latest results shown in SAMUS tend to show that the adaptation strategy could be coupled with external feature extractor may also be useful to increase even more the performances of such  models.

Other figures and additional results in the articles include ablation studies, results on more datasets as well as quantitative and qualitive results for multiple-point input prompts and/or box prompts.

# Conclusion

Several recent works have leveraged SAM architecture and training and tried to modify it in order to improve its capacities on medical imaging tasks. They mainly rely on Low-rank adaptation modules introduced inside the transformer's architecture of SAM, but new ideas, such as using a CNN as local features extractor, may leads to even an greater increase in performances in the future.
