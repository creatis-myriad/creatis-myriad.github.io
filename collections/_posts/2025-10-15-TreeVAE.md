...
&nbsp; 
  ...
  ➔ $$p_{\theta}(z_{\mathcal{P}_l}, \mathcal{P}_l) = p(z_0) \, \prod_{i\in \mathcal{P}_l \backslash \{0\}}{\,p(c_{pa(i) \to i} \mid z_{pa(i)}) \, p(z_i \mid z_{pa(i)})}$$   
...
* ***Modeling of the inference model***  
  ➔ decribed by the variational posterior distribution of both the latent embeddings $$z_{\mathcal{P}_l}$$ and the paths $$\mathcal{P}_l$$  
  ➔ the probability of the root and of the decisions are now conditioned on the sample $$x$$  
  ➔ $$q(z_{\mathcal{P}_l}, \mathcal{P}_l \mid x) = q(z_0 \mid x) \, \prod_{i\in \mathcal{P}_l \backslash \{0\}}{\,q(c_{pa(i) \to i} \mid x) \, q(z_i \mid z_{pa(i)})}$$   
