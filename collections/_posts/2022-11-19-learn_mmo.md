...
Let $$\Gamma(\mathcal{H})$$ be the set of lower semi-continuous convex functions from a real Hilbert space $$\mathcal{H}$$ to $$]-\infty, +\infty]$$. Let $$f$$ and $$g$$ belong to  $$\Gamma(\mathcal{H})$$ and $$\partial f$$ and $$\partial g$$ their Moreau's subdifferential. $$x$$ is a solution if :
...
In this paper, they want to ensure its convergence by learning a neural network that mimics the mathematicals properties of a maximally monotone operator (MMO).


# Highlights

In this article, they propose: 
- A theorem that prove that they can approximate the resolvant of a MMO with a neural network (not detailed)
