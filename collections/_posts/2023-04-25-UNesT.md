...
# Notes
* Code is available on GitHub in Monai's models: [https://github.com/Project-MONAI/model-zoo/tree/dev/models](https://github.com/Project-MONAI/model-zoo/tree/dev/models).

# Highlights
- Introduce a novel hierarchical hybrid 3D U-shape medical segmentation model with Nested Transformers (UNesT).
- Combination of transformer-based encoder with 3D block aggregation function and CNN-based decoder with skip connections enable learning for local behaviors for small structures or small dataset.
...
The authors proposed data-efficient UNesT to solve the potential limitations of transformers.
...
- Skip connections from transformer encoders after being fed to residual blocks are concatenated with the decoder.

**Training strategy:**
All experiments are implemented in PyTorch and MONAI. During the training, an input patch size of $$ 96 \times 96 \times 96 $$ and a cosine warm-up scheduler of 500 steps are used. 5-fold cross validation is done for all datasets and the ensembled model is used for inference.
...
All models are pretrained with 4859 auxiliary pseudo labels and are fine-tuned with 50 manually traced labels. 
...
- They prove that base and large models can scale well when there is more data.  
...
# Conclusions
The authors proposed a novel hierarchical transformer-based medical image segmentation approach (UNesT) with 3D block aggregation module to achieve local communication. This approach efficiently combined global and local information, and successfully solved two critical problems of transformer-based segmentation models: lack of local positional information resulting in sub-optimal performance when handling considerable tissue classes, and the need of large training dataset to achieve good performance.
