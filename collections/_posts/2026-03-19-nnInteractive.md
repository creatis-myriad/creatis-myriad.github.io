...
A **point** interactation $$p \in \mathbb{R}^3 $$  is sampled from the error volume $$V$$ according to a probability distribution weighted by the normalized euclicdean distance from the border. Voxels near the center are favored over those near the boundary.
...
An initial crop of size $$192 \times 192 \times 192$$ is centered around the first user interaction and passed through the model. The resulting prediction is checked for boundary clipping by comparison against the previous prediction (initialized as a null tensor). If the object is truncated, the ROI is expanded by a factor of 1.51.5
1.5, re-cropped, and downsampled to fit the model input. This process repeats until the object is fully contained within the ROI, yielding a complete low-resolution prediction. The prediction is then resized to the original volume size, and local patches are generated to further refine the final segmentation.
...
The implementation is optimized for broad adoption, maintaining **VRAM usage below 10 GB** (under 6 GB for small objects). **Small structures** such as tumors and organs are segmented in **120–200 ms** on an NVIDIA RTX 4090. For **larger objects**, the AutoZoom mechanism iteratively zooms out and refines the segmentation, with inference times reaching up to **1,160 ms ($$\simeq 1.2 sec$$) for a liver CT** and **3,700 ms ($$\simeq 3.7 sec$$)in rare high-resolution cases**.
...
To ensure fair comparison across methods, interactions are simulated like the methods described above.
...
On average, AutoZoom has a modest but noticeable impact, as most test objects are smaller than the patch size ($$192 \times 192 \times 192$$) and so do not require dynamical zoom. 
...
nnInteractive can be **intregrated into established imaging platforms** such as Napari and MITK Workbench.

nnInteractive is benchmarked against expert manual tumor annotation in terms of accuracy and efficiency.
**Segmentation consistency is comparable to inter-expert agreement**, with median Dice scores of 0.842 (resident vs. specialist), 0.794 (specialist vs. nnInteractive), and 0.853 (resident vs. nnInteractive). 
