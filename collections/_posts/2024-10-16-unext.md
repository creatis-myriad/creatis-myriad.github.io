...
# Highlights
* The goal of the paper is to propose a segmentation network with a small the number of parameters and computational complexity which could be implemented in point-of-care applications.
...
This can be a problem for point-of-care imaging, which goal is to test and analyze images at the side of the patient. For example, skin conditions could be detected through a phone camera, and pleural irregularities and other conditions could be checked with point-of-car ultrasound (POCUS).
...
7) The output is processed through a normalzation layer and reshaped as 2D maps.




> A **depthwise convolution** applies a single convolutional filter for each input channe. Each channel is kept separate.
